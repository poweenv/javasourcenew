package lang;
/*
 	 자바에서 문자열 다루기
 	  1) String
 	  2) StringBuffer : 원본 문자열 변경이 가능함 / 멀티쓰레드 안전 , 원본이 변경됨
 	  3) StringBulider: 원본 문자열 변경이 가능함 / StringBuffer 똑같은데 멀티쓰레드 부분만 제거된 클래스
 */
public class StringBufferEx1 {

	public static void main(String[] args) {
		
		StringBuffer sb =new StringBuffer(); //inital capacity of 16 characters : 버퍼의 크기가 16으로 지정됨 지정해주지 않으면
		
		StringBuffer sb2 =new StringBuffer(100); // 버퍼의 크기가 100
		StringBuffer sb3 =new StringBuffer("Hello"); //버퍼의 크기는 문자열 길이만큼
		
		//StringBuffer sb4 = 'hello'; 직접 할당은 불가 
		System.out.println(sb2); // 버퍼의 크기는 잡아놨으나 내용은 없다
		System.out.println(sb3);
		
		//append() : 원본 문자열 뒤에 추가
//		sb2.append(false);
//		sb2.append("abc");
//		sb2.append('d');
		sb2.append(false).append(" abc ").append('d');
		System.out.println(sb2);
		
		//equals() 를 Object가 넘겨준 상태(주소 비교)로 사용
		StringBuffer sb4 = new StringBuffer("Hello");
		System.out.println("equals() : "+sb3.equals(sb4));//false
		
		//값비교를 해야 한다면? String 으로 변경
		String str1 = sb3.toString();
		String str2 = sb4.toString();
		System.out.println("String equals() : "+str1.equals(str2));
		
		
		
		//delete(int start, int end) : 시작 위치에서 끝 위치-1 사이의 문자를 제거 
		StringBuffer sb5 = new StringBuffer("0123456");
		StringBuffer sb6 = sb5.delete(3, 6);
		System.out.println("StringBuffer sb6 delete() : "+sb6);
		System.out.println("StringBuffer sb5 delete() : "+sb5);
		
		//deleteCharAt(int Index) : index 의 문자 제거
		sb5 = new StringBuffer("0123456");
		sb6 = sb5.deleteCharAt(4);
		System.out.println(sb6);
		System.out.println(sb5);
		
		//insert(int offset, 삽입할 값) : offset 위치에 값을삽입
		sb5.insert(4, "-");
		System.out.println(sb5);
		
		//replace(int start, int end, String str) : 
		sb5 = new StringBuffer("0123456");
		sb5.replace(3, 6, "avc");
		System.out.println("replace() : "+sb5);
		
		
		str2 = "0123456789";
		// 출력 9876543210
		for (int i = str2.length()-1; i >=0 ; i--) {
			System.out.print(str2.charAt(i));
		}
		
		//reverse(): 거꾸로 나열
		sb5= new StringBuffer(str2);
		sb5.reverse();
		System.out.println();
		System.out.println(sb5);
		
		//setCharAt() : 
		sb5 = new StringBuffer("0123456");
		sb5.setCharAt(3, 'c');
		System.out.println(sb5);
	}

}





























